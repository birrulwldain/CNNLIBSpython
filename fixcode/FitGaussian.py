...
class FitGaussian(object):
    ...
    @staticmethod
    def fit_contoh_gaussian(
        wavelength, intensity, highest_peak_index, window=5, maxfev=5000
    ):
        x_zoom, y_zoom, fit_gaussian_curve_zoom, _ = FitGaussian.fit_gaussian_peak(
            wavelength, intensity, highest_peak_index, window, maxfev
        )
        ...
        plt.plot(
            x_zoom,
            fit_gaussian_curve_zoom,
            label=f"Gaussian Fit {wavelength[highest_peak_index]:.2f} nm",
            color="red",
        )
...
for peak_index in peaks:
    x, y, fit_gaussian_curve, _ = FitGaussian.fit_gaussian_peak(
    ...
    if fit_gaussian_curve is not None:
        ...
        plt.plot(
            x, fit_gaussian_curve, label=f"Gaussian Fit {wavelength[peak_index]:.2f} nm"
        )
...
if peaks.size > 0:
    ...
    FitGaussian.fit_contoh_gaussian(
        wavelength, intensity, highest_peak_index, window=5, maxfev=5000
    )
